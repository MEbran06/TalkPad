...
def test_like(test_posts, session, test_user):
    new_vote = models.Like(post_id=test_posts[3].id, user_id=test_user['id'])
    session.add(new_vote)
    session.commit()

def test_likes_on_post(authorized_client, test_posts):
...
def test_like_post_non_exits(authorized_client, test_posts):
    res = authorized_client.post(
